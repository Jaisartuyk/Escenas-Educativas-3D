...
const searchTerms = [
  ...
  'Gonzalez Plaza Elian', 'Saigua Adriana', 'Celi Aron', 'Hurtado Vera',
...
async function run() {
  ...
  console.log('--- ALUMNOS ENCONTRADOS DE LA LISTA (A SALVO) ---');
  Array.from(matchedStudents).forEach(s => console.log(' - ' + s.full_name));

  const { data: parents } = await supabase.from('profiles').select('id, full_name, email').eq('institution_id', inst.id).eq('role', 'parent');
  
  const parentsToSuspend = (parents||[]).filter(p => !safeParentIds.has(p.id));
  const safeParents = (parents||[]).filter(p => safeParentIds.has(p.id));

  console.log('\n--- RESUMEN FINAL ---');
  console.log('Alumnos Encontrados: ' + matchedStudents.size);
  console.log('Padres a Salvo (ID unicos): ' + safeParents.length);
  console.log('Padres a Suspender: ' + parentsToSuspend.length);
}
